...
**Configuration ownership (TASK-027):** `ACCESS_TEAM_DOMAIN`/`ACCESS_AUD` are managed exclusively as Cloudflare Worker dashboard runtime variables (Settings → Runtime variables and secrets), never as `wrangler.jsonc` `vars` — real Access values must never be committed to the repository, and placeholder values must never be substituted in `wrangler.jsonc` merely to satisfy tooling. `wrangler.jsonc` sets `keep_vars: true` specifically so `wrangler deploy` preserves these dashboard-managed variables instead of clearing them; this must not be removed as unused cleanup without revisiting this decision. Because `wrangler types` only derives `Env` from repository configuration, it cannot discover dashboard-only variables — the generated `Env` (`worker-configuration.d.ts`) intentionally has no `ACCESS_TEAM_DOMAIN`/`ACCESS_AUD` properties, and that file must not be hand-edited to add them. Instead, a narrow, application-owned `AccessEnvironment` type (`src/lib/server/auth/CloudflareAccessJwtAuthenticationContext.ts`) declares only the two optional properties the production authentication factory needs, and `App.Platform.env` (`src/app.d.ts`) composes it with the generated `Env` (`Env & AccessEnvironment`) rather than duplicating or replacing generated bindings such as `WATCHLIST_KV`/`ASSETS`. This composition only satisfies the type checker — it establishes no trust. Missing/empty configuration is still validated at runtime and fails closed rather than falling back to any identity, exactly as before this task.
