...
## Generated Cloudflare Types and Worker Secrets

`worker-configuration.d.ts` is generated by `wrangler types` (`npm run gen`)
from `wrangler.jsonc`. Do not manually edit it.

Production's two Access configuration values, `ACCESS_TEAM_DOMAIN` and
`ACCESS_AUD`, are configured only as Cloudflare Worker **Secrets** (Settings
→ Runtime variables and secrets → Type: Secret), never as ordinary
`wrangler.jsonc` `vars` — their real values must never be committed to the
repository. `wrangler.jsonc` declares their *names* (never values) via
`secrets: { required: [...] }`, which the installed Wrangler version uses to
generate them as part of the standard `Env`. Do not reintroduce
fake/placeholder values into `wrangler.jsonc` `vars` merely to make such
variables appear in the generated `Env`, and do not hand-edit
`worker-configuration.d.ts`. Even where generated types describe a Secret as
a required `string`, that is a compile-time declaration only — it does not
prove the value is actually configured in a given deployment, so runtime
fail-closed validation of these values must remain in application code.
